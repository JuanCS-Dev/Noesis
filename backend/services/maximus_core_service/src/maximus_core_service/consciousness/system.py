...
from maximus_core_service.consciousness.esgt.coordinator import ESGTCoordinator, TriggerConditions, ESGTEvent, ESGTPhase
...
# SINGULARIDADE: Gemini Client for Language Motor
try:
...
class ConsciousnessSystem:
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            # SINGULARIDADE: Create GeminiClient as Language Motor
            gemini_client = None
            if GeminiClient is not None:
                try:
                    gemini_client = GeminiClient()
                    logger.info("  ✅ GeminiClient initialized (Language Motor active)")
                except Exception as e:
                    logger.warning(f"  ⚠️  GeminiClient unavailable: {e} (using fallback)")
            else:
                logger.info("  ⚠️  GeminiClient not installed (using fallback narrative)")

            self.consciousness_bridge = ConsciousnessBridge(
                unified_self=self.self_concept,
                llm_client=gemini_client  # SINGULARIDADE: Inject Language Motor
            )
            ...
            if await self.episodic_memory.is_available():
                ...
                logger.info(f"  ✅ Episodic Memory connected ({memory_count} memories)")
            else:
    ...
    async def process_input(
        ...
        logger.info(f"[SINGULARIDADE] Input salience: {salience_raw:.3f}")
        ...
        logger.info(f"[SINGULARIDADE] ESGT ignition: phase={event.current_phase.value}, coherence={coherence:.3f}")
        ...
        if self.consciousness_bridge and event.was_successful():
            ...
            logger.info(f"[SINGULARIDADE] Bridge response: meta_level={response.meta_awareness_level:.2f}")
            ...
            if self.episodic_memory:
                try:
                    ...
                    logger.debug(f"[MNEMOSYNE] Stored memory for event {event.event_id[:8]}")
                except Exception as e:
                    logger.warning(f"[MNEMOSYNE] Memory store failed: {e}")
        ...
        return IntrospectiveResponse(
            event_id=event.event_id,
            narrative=f"Ignição incompleta (fase: {event.current_phase.value}). Coerência: {coherence:.2f}. Razão: {event.failure_reason or 'desconhecida'}",
    ...
    async def process_input_streaming(
        ...
        try:
            ...
            if self.consciousness_bridge and event.was_successful():
                ...
                # Fallback narrative for failed ignition
            ...
            logger.error(f"[MAXIMUS STREAMING] Error: {e}")
            yield {
                "type": "error",
                "message": str(e),
                "timestamp": datetime.now().isoformat()
